...
function Model({ url }: { url: string }) {
  ...
  const handleMouseEnter = () => {
    if (actions && actions["hammerUp"] && !hasPlayedUpRef.current) {
      actions["hammerUp"].reset()
      actions["hammerUp"].setLoop(THREE.LoopOnce, 1) // Play once
      actions["hammerUp"].clampWhenFinished = true  // Stop at last frame
      actions["hammerUp"].play()
