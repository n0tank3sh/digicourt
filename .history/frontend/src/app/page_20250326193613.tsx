...
function Model({ url }: { url: string }) {
  ...
  const handleMouseEnter = () => {
    if (actions && actions["hammer"] && !hasPlayedUpRef.current) {
      actions["hammer"].reset()
      actions["hammer"].setLoop(THREE.LoopOnce, 1) // Play once
      actions["hammer"].clampWhenFinished = true  // Stop at last frame
      actions["hammer"].play()
