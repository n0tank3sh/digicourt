...
function Model({ url }: { url: string }) {
  ...
  const handleMouseEnter = () => {
    if (actions && actions["hammerAction"] && !hasPlayedRef.current) {
      actions["hammerAction"].reset().play() // Play the "hammer up" animation
  ...
  const handleMouseLeave = () => {
    if (actions && actions["hammerStrike"]) {
      actions["hammerStrike"].reset().play() // Play the "hammer strike" animation
      hasPlayedRef.current = false // Reset the state for the next interaction
    }
  }
